...
function calculateGoldProfit(buildLvls, taxesMultiplier, gainMultiplier = 1) {
  ...
  const resourceGainByType = getResourceGainByType(peopleCountByBuildType, taxesMultiplier, gainMultiplier);
...
function getResourceGainByType(peopleCountByBuildType, taxesMultiplier, gainMultiplier) {
  const resourceGainPerType = Object.keys(peopleCountByBuildType).reduce((resourceGain, buildType) => {
    ...
    const resourcePerHuman = (RESOURCE_GAIN_PER_HUMAN_BY_BUILD_TYPE[buildType] / taxesMultiplier) * gainMultiplier;
