 ...
 */
function calculateGoldProfit(buildLvls, taxesMultiplier, gainMultiplier = 1) {
  const peopleCountByBuildType = getPeopleCountByBuildType(buildLvls);

  const resourceGainByType = getResourceGainByType({
    peopleCountByBuildType,
    buildLvls,
    taxesMultiplier,
    gainMultiplier,
  });
  ...
  const goldGainedFromTaxes = Math.floor(peopleCountByBuildType.houses * taxes * gainMultiplier);
  ...
  const goldInGainedResources = rockBuyPrice * rockGained * 0.75 + woodBuyPrice * woodGained * 0.75;
...
function getResourcePerHuman(buildType, buildLvl) {
  if (!["mine", "sawmill"].includes(buildType)) return RESOURCE_GAIN_PER_HUMAN_BY_BUILD_TYPE[buildType];

  // чем выше уровень тем меньше процентное увеличение
  // но сам апгрейд выше

  return Math.max(buildLvl * 0.07, 1);
}

function getResourceGainByType({ peopleCountByBuildType, buildLvls, taxesMultiplier, gainMultiplier }) {
  const resourceGainPerType = Object.keys(peopleCountByBuildType).reduce((resourceGain, buildType) => {
    ...
    const resourcePerHuman = (getResourcePerHuman(buildType, buildLvl) / taxesMultiplier) * gainMultiplier;
