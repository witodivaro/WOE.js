const { getBuildCostsByType, calculateBuildCost } = require("../calculateBuildsCost");
...
function generateTownhallMatrix(buildLvls) {
  ...
  for (let housesIndex = 0; housesIndex < MAX_HOUSES_INDEX; housesIndex++) {
    ...
    for (let townhallIndex = 0; townhallIndex < MAX_TOWNHALL_INDEX; townhallIndex++) {
      ...
      townhallCost = calculateBuildCost(townhallLvl, "townhall");
  ...
  writeDataFile(`townhall-gold-max-taxes.csv`, goldCsv);
  writeDataFile(`townhall-roi-max-taxes.csv`, roiCsv);
  writeDataFile(`townhall-cost-max-taxes.csv`, costCsv);
}
