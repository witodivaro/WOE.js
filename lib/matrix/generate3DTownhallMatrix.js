...
function generate3DTownhallMatrix({ buildLvls, gainMultiplier }) {
  ...
  const MIN_TOWNHALL_INDEX = 0;
  const MIN_HOUSES_INDEX = 0;
  const MAX_TOWNHALL_INDEX = 350;
  const MAX_HOUSES_INDEX = 350;
  const MAX_FARM_INDEX = 350;
  ...
  for (let housesIndex = MIN_HOUSES_INDEX; housesIndex < MAX_HOUSES_INDEX; housesIndex++) {
    console.log(`Generating 3D Matrix ${((housesIndex / MAX_HOUSES_INDEX) * 100).toFixed(2)} %`);
  ...
  writeDataFile("th-3d-gold.json", JSON.stringify(goldMatrix, null, 2));
  writeDataFile("th-3d-cost.json", JSON.stringify(costMatrix, null, 2));
  writeDataFile("th-3d-roi.json", JSON.stringify(roiMatrix, null, 2));
